...
const Waitlist = () => {
  ...
  return (
    <section className="py-24 bg-gradient-to-br from-[#0D1520] to-[#1a2536] relative border-t border-white/5" id="waitlist">
      ...
      {/* Embedded Success Modal for Home Page Waitlist */}
      {showSuccess && (
...
const PricingPage = ({ setView }) => {
  ...
  const completePayment = () => {
    ...
    setTimeout(() => {
      ...
      setView('signup'); // Proceed to actual sign up after payment
    }, 2000);
  };
  ...
  return (
    <div className="animate-in fade-in duration-500 bg-[#F8FAFC] min-h-screen pt-32 pb-24">
      ...
      {/* External Payment Gateway Redirect Modal */}
      {checkoutPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
...
const ListPropertyWizard = ({ setView }) => {
  ...
  const finish = () => {
    setView('signup'); // Proceed to actual signup/dashboard after creating property profile
  };
...
const AboutPage = ({ setView, openWaitlist }) => {
  return (
    <div className="animate-in fade-in duration-500 bg-[#F8FAFC]">
      {/* About Hero */}
      <section className="relative pt-40 pb-16 overflow-hidden bg-[#F8FAFC]">
        <div className="max-w-5xl mx-auto px-6 text-center relative z-10 mb-16">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <button onClick={() => openWaitlist()} className="w-full sm:w-auto bg-transparent border-2 border-[#3B82F6] text-[#3B82F6] px-8 py-4 rounded-xl font-bold text-lg hover:bg-blue-50 transition-all flex items-center justify-center gap-2">
              Join the Waitlist
            </button>
          </div>
        </div>
...
export default function App() {
  const [currentView, setCurrentView] = useState('home'); // 'home', 'renters', 'landlords', 'browse', 'hq', 'about', 'contact', 'login', 'signup', 'dashboard', 'pricing', 'list-property'
  const [dashboardRole, setDashboardRole] = useState('tenant'); // 'tenant', 'landlord', 'enterprise'
  const [isWaitlistOpen, setIsWaitlistOpen] = useState(false);
  const [policyModal, setPolicyModal] = useState(null); // 'privacy', 'terms', 'cookie', or null
